    ...
    return (
        <div className="w-full bg-black text-white min-h-screen pt-24 md:pt-32 pb-24">
            <div className="max-w-[1200px] mx-auto px-6 md:px-12">
                {/* Header */}
                <h1 className="font-serif text-5xl md:text-7xl font-light tracking-tight mb-4">Journal</h1>
                <p className="editorial-body text-sm text-white/50 mb-12">
                    Behind the scenes, tips, and stories from the fashion world.
                </p>
                ...
                <div className="flex flex-wrap gap-3 mb-16 border-b border-white/10 pb-4">
                    {journalCategories.map((cat) => (
                        <button
                            ...
                            className={`font-sans text-[10px] tracking-[0.2em] uppercase px-3 py-2 transition-colors ${activeCategory === cat
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                    {filtered.map((post) => (
                        <Link key={post.slug} href={`/journal/${post.slug}`} className="group">
                            ...
                            <h2 className="font-serif text-2xl md:text-3xl font-light group-hover:text-white/80 transition-colors leading-tight mb-3">
                                {post.title}
                            </h2>
